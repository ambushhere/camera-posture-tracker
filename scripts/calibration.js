...
async function startCalibration() {
    ...
    let count = 3;
    updateStatus('Займите правильную позу...', 'var(--warning-color)');
...
function captureReference() {
    ...
    const captureHandler = (e) => {
        ...
        if (results && results.poseLandmarks) {
            ...
            isCalibrating = false;
            updateStatus('Калибровка завершена ✅', 'var(--success-color)');
            document.getElementById('btn-start').disabled = false;
            document.getElementById('metrics-panel').classList.remove('hidden');

            notifications.show('Эталон зафиксирован! Можно начинать мониторинг.', 'success');
...
function resetApp() {
    ...
    updateStatus('Ожидание калибровки', 'rgba(0,0,0,0.6)');
}
